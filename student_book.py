...
try:
 if(btn=="Save"):
  ...
  if(k==1):
   ...
   print("<script>alert('Welldone Boss..Saved On student_save form.....')</script>")
   print("<script>window.open('Student_book_save.html','_self')</script>")
 ...
 if(btn=="Update"):
  ...
  print("<script>window.open('Student_books_update.html','_self')</script>")
  #Delete Button:
 if(btn=="Delete"):
  client=pymongo.MongoClient("mongodb://localhost:27017/")
  db=client['library']
...
#All Search Buttton:
 if(f.getvalue("b1")=="Allsearch"):
  ...
  db=client['library']
  collection=db['Student_book']
  print("<center><table border=15 cellpadding=5 bgcolor=skyblue><tr><th>Book ID</th><th>Book tittle</th><th>Book edition</th><th>Book author</th><th>Book Publisher</th><th> Book Copies</th><th> Book source </th><th>Book Cost</th><th>Boook Remarks </th></tr>")
  for x in collection.find({}):
